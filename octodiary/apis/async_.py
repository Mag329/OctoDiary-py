...
from octodiary.apis.base import AsyncBaseAPI
...
class AsyncMobileAPI(AsyncBaseAPI):
    ...
    async def login(self, username: str, password: str) -> str:
        ...
        if self.system == Systems.MYSCHOOL:
            ...
            get_variables = await (
                await self._login_info["session"].post(
    ...
    async def esia_login(self, username: str, password: str) -> Union[str, bool]:
        """
        Login with ESIA (gosuslugi.ru) and get api-token.

        If you get ``False`` -> it means that you have MFA, use method ``.esia_enter_MFA(code=<CODE>)``.
        """
        ...
        await self.__session_login.get(
            url=BaseURL(type=URLTypes.AUTH, system=Systems.MYSCHOOL) + "/aas/oauth2/config"
        )
        login_response = await self.__session_login.post(
            url=BaseURL(type=URLTypes.AUTH, system=Systems.MYSCHOOL) + "/aas/oauth2/api/login",
            json={
                "login": username,
                "password": password
            }
    ...
    async def get_users_profile_info(self) -> list[UsersProfilesInfo]:
        """
    ...
    async def get_lesson_schedule_item(
            self,
            profile_id: int,
            lesson_id: int,
            student_id: int,
    ...
    async def get_rating_rank_short(
            self,
            profile_id: int,
            person_id: str,
            begin_date: Optional[date] = None,
    ...
    async def get_class_members(self, class_unit_id: int, per_page: int = 2147483647, types: str = "student") -> ClassMembers:
        """
    ...
    async def get_subjects_marks(self, profile_id: int, student_id: int) -> SubjectsMarks:
        """
...
class AsyncWebAPI(AsyncBaseAPI):
    ...
    async def login(self, username: str, password: str) -> str:
        ...
        if self.system == Systems.MYSCHOOL:
            ...
            self.code_verifier = "".join(random.choices(string.ascii_letters + string.digits + "_-", k=80))
            self.code_challenge = base64.urlsafe_b64encode(
                hashlib.sha256(self.code_verifier.encode()).digest()
            ).decode().replace("=", "")
            proof_of_work = self._resolve_proof_of_work(
    ...
    async def get_user_info(self) -> web.UserInfo:
        """
        Get user info
    ...
    async def get_person_data(self, person_id: str) -> web.PersonData:
        """
        Get person data
        ...
        return await self.request(
            ...
            model=web.PersonData,
        )

    async def get_all_roles_global(self) -> list[web.Role]:
        """
    ...
    async def get_children(self, sso_id: str, timeout: int = 10) -> web.UserChildren:
        """
        Get children
